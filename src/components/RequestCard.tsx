  ...
  return (
    <Card className={cn(
      ...
      <Dialog>
        ...
        <DialogContent className="glass-card border-white/10 max-w-3xl max-h-[90vh] flex flex-col p-0 rounded-[32px] shadow-[0_0_80px_rgba(0,0,0,0.5)] overflow-hidden">
          ...
          <ScrollArea className="flex-1 min-h-0 bg-[#0a0a0c]">
            <div className="p-6 md:p-12 space-y-12 md:space-y-16 pb-32">
          ...
          <div className="p-6 md:p-8 border-t border-white/5 bg-[#0a0a0c]/80 backdrop-blur-md shrink-0">
            {!isOwner && !isCompleted && (
              <Dialog open={isHelpModalOpen} onOpenChange={setIsHelpModalOpen}>
                <DialogTrigger asChild>
                  <Button className="w-full h-16 bg-primary hover:bg-primary/90 text-black font-black rounded-2xl gold-glow uppercase tracking-[0.2em] text-sm gap-3 transition-all shadow-[0_10px_40px_rgba(244,201,93,0.15)]">
                    <Heart size={20} className="fill-current" />
                    Help @{requestor} Now
                  </Button>
                </DialogTrigger>
                <DialogContent className="glass-card border-white/10 max-w-sm p-8 space-y-8 shadow-2xl rounded-[32px] z-[60]">
                  <DialogHeader>
            ...
            {isOwner && !isCompleted && (
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="outline" className="w-full h-16 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 font-black rounded-2xl gap-3 uppercase tracking-widest text-sm" disabled={isProcessing}>
                    {isProcessing ? <Loader2 className="animate-spin" size={20} /> : <><CheckCircle2 size={20} /> Mark Request as Done</>}
                  </Button>
                </AlertDialogTrigger>
                <AlertDialogContent className="glass-card border-white/10 p-8 rounded-[32px] z-[60]">
                  <AlertDialogHeader>
            ...
            {isCompleted && (
              <div className="h-16 flex items-center justify-center rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
                <span className="text-sm font-black text-emerald-400 flex items-center gap-2 uppercase tracking-[0.2em]"><CheckCircle2 size={18} /> This request is completed</span>
              </div>
            )}
          </div>
        </DialogContent>
      ...
      <div className={cn("flex flex-col gap-3", variant === 'list' ? "p-6 border-l border-white/5 shrink-0 justify-center min-w-[160px]" : "p-6 pt-0")}>
        {!isOwner && !isCompleted && (
          <Button variant="outline" className={cn("w-full border-white/10 hover:bg-white/5 text-foreground font-black rounded-xl uppercase tracking-widest gap-2 transition-all", variant === 'list' ? "h-12 text-[11px]" : "h-14 text-sm")}>
            <Heart size={14} className="text-primary fill-primary" /> View Details & Help
          </Button>
        )}

        {isOwner && !isCompleted && (
          <Button variant="outline" className={cn("w-full border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 font-black rounded-xl gap-2 uppercase tracking-widest", variant === 'list' ? "h-12 text-[10px]" : "h-14 text-xs")}>
            <CheckCircle2 size={14} /> View Progress
          </Button>
        )}
