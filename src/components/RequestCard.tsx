...
import { Heart, X, Loader2, CheckCircle2, Zap, Sparkles, Image as ImageIcon, MessageSquare, Quote, ArrowRight } from 'lucide-react';
...
const RequestCard: React.FC<RequestCardProps> = ({
  ...
  const cardContent = (
    <CardContent className={cn("p-0 cursor-pointer hover:bg-white/[0.02] transition-colors flex-1", variant === 'list' && "flex flex-col md:flex-row")}>
      {proofUrl && (
        <div className={cn(
          "overflow-hidden relative border-white/5",
          variant === 'grid' ? "w-full aspect-[21/9] border-b" : "w-full md:w-48 aspect-video md:aspect-square shrink-0 border-b md:border-b-0 md:border-r"
        )}>
      ...
      <div className={cn("p-6 space-y-4 flex-1", variant === 'list' && "flex flex-col justify-center")}>
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <div className="text-[10px] font-black text-muted-foreground flex items-center gap-1 uppercase tracking-wider">
              @{requestor}
            </div>
          </div>
        </div>
  ...
  return (
    <Dialog>
      <Card className={cn(
        ...
        {variant === 'list' && <div className={cn("w-1 transition-all duration-1000 shrink-0", isFunded ? "bg-emerald-500" : getCategoryColor().split(' ')[1])} style={{ height: `${progress}%` }} />}
        ...
        {variant === 'list' && (
          <div className="p-6 border-l border-white/5 flex items-center shrink-0">
             {!isOwner && status === 'Open' ? (
               <Button 
                 size="sm"
                 className="h-11 px-6 bg-primary hover:bg-primary/90 text-black font-black rounded-xl gold-glow uppercase tracking-widest text-[10px] gap-2" 
                 onClick={(e) => {
                   e.stopPropagation();
               ...
               <DialogTrigger asChild>
                 <Button variant="ghost" size="icon" className="h-11 w-11 rounded-xl bg-white/5 hover:bg-white/10">
                   <ArrowRight size={18} />
                 </Button>
               </DialogTrigger>
             )}
            ...
            {!isOwner && status === 'Open' && (
              <div className="w-full space-y-3">
                {!showContribute ? (
                  ...
                  <div className="space-y-5 bg-white/5 p-5 rounded-2xl border border-primary/20 animate-in zoom-in-95 duration-200">
                    <div className="flex items-center justify-between">
                      <p className="text-[11px] font-black uppercase tracking-widest text-primary">Quick Help</p>
                      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => setShowContribute(false)}>
                        <X size={14} />
                      </Button>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {quickAmounts.map(amt => (
                        <Button 
                          ...
                          className={cn("h-8 text-[10px] font-black", contributionAmount === amt.toString() ? "bg-primary text-black" : "")}
                        >
                          {amt}
                        </Button>
                      ))}
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        className="bg-white/5 border-white/10 h-11 text-sm font-black"
                      />
                      <Select value={contributionToken} onValueChange={(v: TokenSymbol) => setContributionToken(v)}>
                        <SelectTrigger className="w-24 h-11 bg-white/5 border-white/10 text-[10px] font-black">
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    >
                      {isProcessing ? <Loader2 className="animate-spin" size={20} /> : `Confirm Send`}
                    </Button>
                  </div>
                )}
              </div>
